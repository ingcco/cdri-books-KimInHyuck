...
- **데이터 리셋(새 검색)으로 스크롤을 top에서 다시 시작 = 명령형 ❌, 리스트 컴포넌트 `key` 재마운트 ⭕**: `scrollRef.scrollTo(0)`·`virtualizer.scrollToOffset(0)`는 안 먹는다. 새 검색 시 결과 섹션이 `isFetched` 게이트로 언마운트→리마운트되면 persistent virtualizer가 저장한 **이전 offset을 새 스크롤 엘리먼트에 복원**하기 때문. 리스트를 검색키로 `key` 부여해 virtualizer 자체를 새로 생성한다 — `<BookResultList key={\`${filters.q}|${filters.target}\`} />`. ※ **가상 스크롤 한정** — 일반 입력 버퍼 리셋은 key가 아니라 render-time `useState`(위 "리셋은 effect 말고 — `key` 또는 렌더 중 `useState` 비교" 참조).
