...
- **Playwright** — 검색부터 관심 도서 담기까지, 실제 브라우저에서 사용자 흐름을 그대로 확인합니다.
...
- 관심 도서 저장 로직은 검색 화면과 관심 도서 화면이 함께 써서 공유 폴더(`lib/favorites`)에 두었습니다.
...
| E2E  | 검색→관심 도서 담기 흐름, Figma 디자인 확인        | 15   | `pnpm test:e2e`         |
...
| ------------------------ | ----------- | ------------- | -------------- | --- |
| 도서 검색 (`/`)          | 100         | 95            | 100            | 91  |
| 관심 도서 (`/favorites`) | 100         | 100           | 100            | 92  |
